...
namespace ImGuiUtils
{
    ...
    public:
        ...
        void RenderTimings(int graphWidth, int legendWidth, int height, int frameIndexOffset)
        {
            ...
            RenderLegend(
                drawList,
                widgetPos,
                glm::vec2(legendWidth, height + 16 * this->taskStats.size() - 16),
                frameIndexOffset);
    ...
    private:
            ...
            ImDrawList* drawList, glm::vec2 graphPos, glm::vec2 graphSize, size_t frameIndexOffset)
        {
            ...
            {
                size_t frameIndex =
                    (currFrameIndex - frameIndexOffset - 1 - frameNumber + 2 * frames.size())
                    % frames.size();

                glm::vec2 framePos =
                    graphPos
                    + glm::vec2(
                        graphSize.x - 1 - frameWidth - (frameWidth + frameSpacing) * frameNumber,
                        graphSize.y - 1);
                ...
                for (const auto& task : frame.tasks)
                {
                    ...
                    // taskMaxCosts[task.name] = std::max(taskMaxCosts[task.name], task.endTime -
                    // task.startTime);
            ...
            size_t      frameIndexOffset)
        {
            ...
            for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
            {
              ...
              if (it != priorityEntries.end() && !it->second.isUsed)
              {
                ...
                glm::vec2 markerLeftRectMin = legendPos + glm::vec2(markerLeftRectMargin,
            legendSize.y); glm::vec2 markerLeftRectMax = markerLeftRectMin +
            glm::vec2(markerLeftRectWidth, 0.0f); markerLeftRectMin.y -= taskStartHeight;
                markerLeftRectMax.y -= taskEndHeight;
            ...
            markerLeftRectMax, markerRightRectMin, markerRightRectMax, task.color);
                ...
                Text(drawList, markerRightRectMax + textMargin + glm::vec2(nameOffset, 0.0f),
            textColor, (std::string("ms] ") + task.name).c_str()); shownTasksCount++;
              }
            }*/
            ...
            glm::vec2(0.0f, (markerHeight + itemSpacing) * priorityTaskIndex); Rect(drawList,
            taskPos, taskPos + glm::vec2(markerHeight, -markerHeight), task.color, true);
        ...
        static void Triangle(
            ImDrawList* drawList, std::array<glm::vec2, 3> points, uint32_t col, bool filled = true)
        {
            if (filled)
            {
    ...
    public:
        ...
        void Render()
        {
            ...
            {
                float fpsDeltaTime =
                    std::chrono::duration<float>(currFrameTime - prevFpsFrameTime).count();
            ...
            if (graphHeight * 2 + sizeMargin + sizeMargin < canvasSize.y)
            {
                ...
                ImGui::SliderFloat(
                    "Transparency", &ImGui::GetStyle().Colors[ImGuiCol_WindowBg].w, 0.0f, 1.0f);
                ImGui::Columns(1);
            }
            if (!stopProfiling)
            {
