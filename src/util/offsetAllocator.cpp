...
namespace OffsetAllocator
{
    ...
    Allocation Allocator::allocate(uint32 size)
    {
        ...
        if (leafBinIndex == Allocation::NO_SPACE)
        {
            ...
            // All leaf bins here fit the alloc, since the top bin was rounded up. Start leaf search
            // from bit 0. NOTE: This search can't fail since at least one leaf bit was set because
            // the top bit was set.
