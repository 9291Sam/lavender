...
namespace OffsetAllocator
{
    ...
    namespace SmallFloat
    {
        ...
        // (piecewise linear log approx) This ensures that for each size class,
        ...
        uint32 uintToFloatRoundUp(uint32 size)
        {
            ...
            {
                // Normalized: Hidden high bit always 1. Not stored. Just like
                // float.
        ...
        , m_freeOffset(other.m_freeOffset)
    {
        ...
        memcpy(
            m_binIndices,
            other.m_binIndices,
            sizeof(NodeIndex) * NUM_LEAF_BINS);
    ...
    Allocation Allocator::allocate(uint32 size)
    {
        ...
        {
            topBinIndex =
                findLowestSetBitAfter(m_usedBinsTop, minTopBinIndex + 1);
            ...
            // All leaf bins here fit the alloc, since the top bin was rounded
            // up. Start leaf search from bit 0. NOTE: This search can't fail
            // since at least one leaf bit was set because the top bit was set.
        ...
        if (reminderSize > 0)
        {
            ...
            // Link nodes next to each other so that we can merge them later if
            // both are free And update the old next neighbor to point to the
            // new node (in middle)
...
#ifdef DEBUG_VERBOSE
        printf(
            "Putting node %u into freelist[%u] (free)\n",
            nodeIndex,
            m_freeOffset + 1);
#endif
...
#ifdef DEBUG_VERBOSE
        printf(
            "Free storage: %u (+%u) (insertNodeIntoBin)\n",
            m_freeStorage,
            size);
#endif
...
#ifdef DEBUG_VERBOSE
        printf(
            "Free storage: %u (-%u) (removeNodeFromBin)\n",
            m_freeStorage,
            node.dataSize);
#endif
    }
    ...
    StorageReport Allocator::storageReport() const
    {
        ...
        if (m_freeOffset > 0)
        {
            ...
            if (m_usedBinsTop)
            {
                ...
                uint32 leafBinIndex =
                    31 - lzcnt_nonzero(m_usedBins[topBinIndex]);
    ...
    StorageReportFull Allocator::storageReportFull() const
    {
        ...
        for (uint32 i = 0; i < NUM_LEAF_BINS; i++)
        {
            ...
            report.freeRegions[i] = {
                .size = SmallFloat::floatToUint(i), .count = count};
        }
        return report;
    }
} // namespace OffsetAllocator
